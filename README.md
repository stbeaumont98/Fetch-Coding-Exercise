...
Since the number in the name corresponds to its id, I've opted to sort by id in the code; however, it's probably safe to assume that that won't be the case in a real-world scenario, and ```it.id``` on [line 45 of MainScreen.kt](https://github.com/stbeaumont98/Fetch-Coding-Exercise/blob/master/app/src/main/java/com/example/fetchexercise/ui/main/MainScreen.kt#L45) can be changed to ```it.name``` for alphabetical sorting.
